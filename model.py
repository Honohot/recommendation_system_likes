...
def load_features() -> pd.DataFrame:
    df = pd.read_sql("SELECT * FROM o_gurylev_14_features_lesson_22", con=engine)
    return df
...
vectorized_text_df = pd.read_sql("SELECT * FROM o_gurylev_14_lesson_10_vecs", con=engine)
...
def recommended_posts(id: int, time: datetime, limit: int = 5) -> List[PostGet]:
    ...
    # Присоединяем фичи к датафрейму для предикта
    df_predict = df_predict.merge(df_with_user_features, on='user_id', how='left') # Фичи юзера
    df_predict = df_predict.merge(vectorized_text_df, on='post_id', how='left') # Фичи постов
